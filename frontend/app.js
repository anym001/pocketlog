  ...
  `
     ...
     <input type="color" value="${editingCatColor}" onchange="pickCatColor(this.value)" aria-label="${_escAttr(tr('categories.customColor'))}">
      ...
      // Sums the matching transactions for one goal and returns display-ready
      // figures. Cents are summed as integers (Math.round) so the celebration
      // threshold doesn't drift on long histories.
      ...
      function renderGoalColorSwatches() {
        const presets = [...CAT_COLOR_PRESETS];
        const hasCurrent = presets.some(
          (p) => p.hex.toLowerCase() === editingGoalColor.toLowerCase()
        );
     ...
     <input type="color" value="${editingGoalColor}" onchange="pickGoalColor(this.value)" aria-label="${_escAttr(tr('categories.customColor'))}">
      ...
      function openGoalModal(id) {
        ...
        if (id) {
          ...
          const now = new Date();
          document.getElementById('goalEditStartDate').value = _iso(now.getFullYear(), now.getMonth(), now.getDate());
