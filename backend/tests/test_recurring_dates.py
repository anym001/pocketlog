...
``RecurringRule`` directly and call the helpers in ``app.recurring_dates``.
...
from app import models, recurring_dates
...
def test_clamp_day_31_in_february_non_leap():
    assert recurring_dates._clamp_day(2026, 2, 31) == date(2026, 2, 28)


def test_clamp_day_31_in_february_leap():
    assert recurring_dates._clamp_day(2024, 2, 31) == date(2024, 2, 29)


def test_clamp_day_31_in_april():
    assert recurring_dates._clamp_day(2026, 4, 31) == date(2026, 4, 30)


def test_clamp_day_29_in_february_non_leap():
    assert recurring_dates._clamp_day(2026, 2, 29) == date(2026, 2, 28)


def test_clamp_day_15_in_april_passes_through():
    assert recurring_dates._clamp_day(2026, 4, 15) == date(2026, 4, 15)
...
def test_monthly_advance_keeps_anchor_day():
    ...
    assert recurring_dates.next_occurrence(r, date(2026, 1, 15)) == date(2026, 2, 15)


def test_monthly_advance_clamps_day_31():
    r = _rule(frequency="monthly", day_of_month=31, interval=1)
    # Jan 31 → Feb 28 (non-leap).
    assert recurring_dates.next_occurrence(r, date(2026, 1, 31)) == date(2026, 2, 28)


def test_interval_2_monthly_skips_a_month():
    r = _rule(frequency="monthly", day_of_month=1, interval=2)
    assert recurring_dates.next_occurrence(r, date(2026, 3, 1)) == date(2026, 5, 1)
...
def test_quarterly_adds_three_months():
    ...
    assert recurring_dates.next_occurrence(r, date(2026, 1, 15)) == date(2026, 4, 15)
...
def test_yearly_feb_29_clamps_off_leap():
    ...
    assert recurring_dates.next_occurrence(r, date(2024, 2, 29)) == date(2025, 2, 28)
...
def test_weekly_advance_respects_weekday():
    ...
    assert recurring_dates.next_occurrence(r, date(2026, 1, 7)) == date(2026, 1, 14)
...
def test_weekly_advance_interval_2():
    ...
    assert recurring_dates.next_occurrence(r, date(2026, 1, 5)) == date(2026, 1, 19)
...
def test_daily_interval_n():
    ...
    assert recurring_dates.next_occurrence(r, date(2026, 1, 1)) == date(2026, 1, 4)
...
def test_end_date_terminates_returns_none():
    ...
    assert recurring_dates.next_occurrence(r, date(2026, 2, 1)) is None
...
def test_max_occurrences_terminates_returns_none():
    ...
    assert recurring_dates.next_occurrence(r, date(2026, 1, 1)) is None
...
def test_first_occurrence_in_future_returns_start_date_clamped():
    ...
    assert recurring_dates.first_occurrence_on_or_after(r, date(2026, 6, 1)) == date(
        2026, 6, 15
    )
...
def test_first_occurrence_on_anchor_returns_anchor_for_daily():
    ...
    assert recurring_dates.first_occurrence_on_or_after(r, date(2026, 6, 7)) == date(
        2026, 6, 7
    )
...
def test_first_occurrence_advances_when_dom_already_past_in_anchor_month():
    ...
    assert recurring_dates.first_occurrence_on_or_after(r, date(2026, 6, 20)) == date(
        2026, 7, 5
    )
...
def test_first_occurrence_returns_none_when_max_already_consumed():
    ...
    assert recurring_dates.first_occurrence_on_or_after(r, date(2026, 1, 1)) is None
...
def test_occurrences_until_respects_today():
    ...
    dates = recurring_dates.occurrences_until(
...
def test_occurrences_until_returns_empty_when_paused():
    ...
    assert (
        recurring_dates.occurrences_until(r, date(2026, 3, 1), limit=10, skips=set())
        == []
    )
...
def test_monthly_advance_crosses_year_boundary():
    r = _rule(frequency="monthly", day_of_month=15, interval=1)
    assert recurring_dates.next_occurrence(r, date(2026, 12, 15)) == date(2027, 1, 15)


def test_quarterly_advance_crosses_year_boundary():
    r = _rule(frequency="quarterly", day_of_month=15, interval=1)
    # Nov 15 + 3 months → Feb 15 of the next year.
    assert recurring_dates.next_occurrence(r, date(2026, 11, 15)) == date(2027, 2, 15)


def test_monthly_interval_12_lands_one_year_later():
    r = _rule(frequency="monthly", day_of_month=10, interval=12)
    assert recurring_dates.next_occurrence(r, date(2026, 6, 10)) == date(2027, 6, 10)
